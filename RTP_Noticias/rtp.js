// Dictionary of RSS feed categories and their corresponding RTP URLs
const feeds = {
    'ultimas': 'https://www.rtp.pt/noticias/rss/ultimas',
    ...
    'videos': 'https://www.rtp.pt/noticias/rss/videos',
    'audios': 'https://www.rtp.pt/noticias/rss/audios'
};
...
async function loadCategory(category) {
  ...
  try {
    ...
    const items = Array.from(xml.querySelectorAll('item')).slice(0, 25); // Take first 25 items
    ...
    document.getElementById('news-section').innerHTML = `<p>Erro ao carregar notícias. <button onclick="loadCategory('${category}')">Repetir</button></p>`;
    console.error('RSS Error:', err);
  }
}

// Load the "ultimas" category on first page load
